...
class smb {
            ...
            switch ($tag) {
                ...
                case 'files':
                    ...
                    $i = ($name <> '.' && $name <> '..')
                        ...
                        : array();
                    break;
                case 'error':   throw new Exception( $regs[1] );
            }
            if ($i) switch ($i[1]) {
                case 'file':
        ...
        switch ($pu['type']) {
            case 'host':
                if ($o = smb::look ($pu))
                   ...
                   trigger_error ("url_stat(): list failed", E_USER_WARNING);
                break;
            case 'share':
                if ($o = smb::look ($pu)) {
                   $found = FALSE;
                   $lshare = trim( strtolower ($pu['share']), '/' );  # fix by Eric Leung
            ...
            case 'path':
                ...
                break;
            default: throw new Exception( 'error in URL');
        }
        return $stat;
    ...
    function unlink ($url) {
        ...
        if ($pu['type'] <> 'path') throw new Exception('unlink(): error in URL');
    ...
    function rename ($url_from, $url_to) {
        ...
        if ($from['host'] <> $to['host'] ||
            ...
            throw new Exception('rename(): FROM & TO must be in same server-share-user-pass-domain');
        }
        if ($from['type'] <> 'path' || $to['type'] <> 'path') {
            throw new Exception('rename(): error in URL');
        }
    ...
    function mkdir ($url, $mode, $options) {
        $pu = smb::parse_url($url);
        if ($pu['type'] <> 'path') throw new Exception('mkdir(): error in URL');
        return smb::execute ('mkdir "'.$pu['path'].'"', $pu);
    }

    function rmdir ($url) {
        $pu = smb::parse_url($url);
        if ($pu['type'] <> 'path') throw new Exception('rmdir(): error in URL');
        smb::clearstatcache ($url);
...
class smb_stream_wrapper extends smb {
            ...
            switch ($pu['type']) {
                ...
                case 'path':
                    ...
                    foreach ($o['info'] as $name => $info) {
                        ...
                        @trigger_error ("dir_opendir(): dir failed for path '{$url}'", E_USER_WARNING);
                    }
                    break;
                default:
                    throw new Exception('dir_opendir(): error in URL', E_USER_ERROR);
            }
            return TRUE;
        } catch (Exception $e) {
            trigger_error( $e->getMessage(), E_USER_WARNING );
    ...
    function stream_open ($url, $mode, $options, $opened_path) {
        try {
            ...
            if ($pu['type'] <> 'path') throw new Exception('stream_open(): error in URL');
            switch ($mode) {
                case 'r':
                case 'r+':
                case 'rb':
