    ...
    */
        ...
        foreach($con2 as $k => $v) {
            ...
            if (preg_match_all("/$patt/ims", $v, $res, PREG_SET_ORDER)) {
                ...
                if (strtoupper($block_word) == $this->block_start_word) {
                    ...
                    $blocks[$parent_name] .= str_replace('\\', '', $this->tag_start_delim) /* ' */ 
                        . '_BLOCK_.' . $cur_block_name . str_replace('\\', '', $this->tag_end_delim); // ' mc bug
    ...
    protected function _getfile ($file) {
        ...
        if (isset($this->filecache[$file])) {
            ...
            if (is_file($file) && is_readable($file) && strlen( $file ) <= 512 ) {
...
class Template_XStack extends Object {
    
    // This class does nothing but provide the non-class programming
    // functionality
