<?php
    ...
    if ( count( $argv ) != 3 ) {
        ...
        echo $term->color( "mv: wrong numbers of arguments! displaying manual...", 'red' ), "\r";
    ...
    try {
        ...
        $needCWD = substr( $src->url, 0, strlen( $cwd ) ) === $cwd ? TRUE : FALSE;
        
        $dst->appendChild( $src );
        
        if ( $needCWD )
            term_set_env( 'path', $parser->decode( $dst->url ) );
        
        echo $term->color( $src->name, 'yellow' ) . ' was successfully moved inside ' . $dst->url, "\r";
